...
export class WasmCodeGenerator extends CodeGenerator {
  ...
  generateWasm(sheets: Sheet[]): WasmCodeGenerationResult {
    ...
    const { inputMap, outputMap, outputSourceMap, collectorTypeMap } = this.extractPortMappings(sheets)
  ...
  private extractPortMappings(sheets: Sheet[]): {
        ...
        if (block.type === 'input_port') {
          ...
          if (feedingConnection) {
            ...
            if (sourceBlock) {
              ...
              // Get the type from the source block
              const sourceType = sourceBlock.parameters?.dataType || 'double'
    ...
    return { inputMap, outputMap, outputSourceMap, collectorTypeMap }
  }
