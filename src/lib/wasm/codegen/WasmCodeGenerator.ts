...
export class WasmCodeGenerator extends CodeGenerator {
  ...
  generateWasm(sheets: Sheet[], parameters: any[] = []): WasmCodeGenerationResult {
    ...
    const { inputMap, outputMap, outputSourceMap, collectorTypeMap, outputTypeMap, inputTypeMap } =
      this.extractPortMappings(sheets, typeMap, flatteningResult.model)
  ...
  private extractPortMappings(
    ...
    inputTypeMap: Map<string, string> // Maps input port name to dataType
  } {
    ...
    /**
     * Resolve a sheet_label_source (From) to the block that feeds the matching
     * sheet_label_sink (Goto). From/Goto are removed during flatten, so WASM
     * must read the Goto's upstream signal — not signals.FromN.
     */
    const resolveSheetLabelSource = (
      sheet: Sheet,
      fromBlock: any
    ): { block: any; portIndex: number } | null => {
      const signalName = fromBlock.parameters?.signalName as string | undefined
      if (!signalName) return null
      const sink = sheet.blocks.find(
        (b: any) =>
          b.type === 'sheet_label_sink' && b.parameters?.signalName === signalName
      )
      if (!sink) return null
      const feed = sheet.connections.find((c: any) => c.targetBlockId === sink.id)
      if (!feed) return null
      const src = sheet.blocks.find((b: any) => b.id === feed.sourceBlockId)
      if (!src) return null
      return { block: src, portIndex: feed.sourcePortIndex }
    }
     ...
     * Recurses when the internal feeder is itself a nested subsystem.
     */
    const resolveSubsystemOutput = (
      ...
      for (const subSheet of subsystemSheets) {
        ...
        // Nested subsystem: recurse so path reaches a leaf signal member
        if (sourceBlock.type === 'subsystem') {
          const nested = resolveSubsystemOutput(sourceBlock, feedingConn.sourcePortIndex)
          return {
            path: nested.path ? `${subsystemName}_${nested.path}` : undefined,
            type: nested.type,
            internalSource: nested.internalSource || sourceBlock,
          }
        }
    ...
    for (const sheet of sheets) {
      for (const block of sheet.blocks) {
        if (block.type === 'input_port') {
          ...
          inputTypeMap.set(portName, block.parameters?.dataType || 'double')
          ...
          if (feedingConnection) {
            let sourceBlock = sheet.blocks.find(b => b.id === feedingConnection.sourceBlockId)
            let sourcePortIndex = feedingConnection.sourcePortIndex

            // Unwind From/Goto so we never emit signals.FromN (removed by flattener)
            if (sourceBlock?.type === 'sheet_label_source') {
              const resolved = resolveSheetLabelSource(sheet, sourceBlock)
              if (resolved) {
                sourceBlock = resolved.block
                sourcePortIndex = resolved.portIndex
              }
            }
            ...
            if (sourceBlock) {
              ...
              if (isSubsystem) {
                ...
                if (outputPorts) {
                  outputPortName = outputPorts[sourcePortIndex] || outputPorts[0]
                }
                ...
                if (isFlattened) {
                  flattenedSignalPath = getFlattenedSignalPath(sourceBlock, sourcePortIndex)
                }
              }
              ...
              const sourceType = getSourceSignalType(sourceBlock, sourcePortIndex)
    ...
    return { inputMap, outputMap, outputSourceMap, collectorTypeMap, outputTypeMap, inputTypeMap }
  }
...
`
    ...
    // Set input function
    wrapper += this.generateWasmSetInputFunction(sanitizedName, inputMap, inputTypeMap)
...
`
   ...
   * Scalar ports assign normally. Vector/matrix ports cannot be set via the
   * scalar JS API — emit a no-op case so the wrapper still compiles.
   */
  private generateWasmSetInputFunction(
    modelName: string,
    inputMap: Map<string, number>,
    inputTypeMap: Map<string, string> = new Map()
  ): string {
    const keepalive = this.wasmOptions.includeEmscriptenExports
      ? 'EMSCRIPTEN_KEEPALIVE\n'
      : ''

    let code = `// Set an input value by index (scalars only; vector/matrix ports are no-ops)
${keepalive}void wasm_set_input(int index, double value) {
    ...
    sortedInputs.forEach(([portName, index]) => {
      ...
      const dataType = inputTypeMap.get(portName) || 'double'
      if (dataType.includes('[')) {
        code += `        case ${index}: (void)value; break; /* ${sanitizedPort}: ${dataType} — scalar API cannot assign arrays */\n`
      } else {
