// lib/blocks/SquareBlockModule.ts
//
// Element-wise square (Simulink Math Function Operator "square" / u^2):
//   y = u .* u
// Preserves scalar / vector / matrix shape.

import { BlockData } from '@/components/BlockNode'
import { IBlockModule, BlockModuleUtils } from './BlockModule'

export class SquareBlockModule implements IBlockModule {
  generateComputation(block: BlockData, inputs: string[], inputTypes?: string[]): string {
    const outputName = `model->signals.${BlockModuleUtils.sanitizeIdentifier(block.name)}`

    let code = `    // Square (x^2) block: ${block.name}\n`
    ...
    if (typeInfo.isMatrix && typeInfo.rows && typeInfo.cols) {
      code += `    // Matrix element-wise square\n`
      code += `    for (int i = 0; i < ${typeInfo.rows}; i++) {\n`
      code += `        for (int j = 0; j < ${typeInfo.cols}; j++) {\n`
      code += `            ${outputName}[i][j] = (${inputExpr}[i][j]) * (${inputExpr}[i][j]);\n`
      code += `        }\n`
      code += `    }\n`
    } else if (typeInfo.isArray && typeInfo.arraySize) {
      code += `    // Vector element-wise square\n`
      code += `    for (int i = 0; i < ${typeInfo.arraySize}; i++) {\n`
      code += `        ${outputName}[i] = (${inputExpr}[i]) * (${inputExpr}[i]);\n`
      code += `    }\n`
    } else {
      code += `    ${outputName} = (${inputExpr}) * (${inputExpr});\n`
    }
