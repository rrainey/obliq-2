...
import { generateDebugMathPreamble } from './DebugMathRuntime'
...
export interface CodeGenerationOptions {
  ...
  /**
   * When true, emit OBLIQ_DEBUG_MATH helpers: safe divide/mod that abort with
...
export class CodeGenerator {
  ...
  private generateSource(model: any, typeMap: Map<string, string>): string {
    ...
    if (this.options.debugMath) {
      source += generateDebugMathPreamble()
      source += '\n'
    }
    ...
    // Derivatives function (if needed)
    const rk4Generator = new RK4Generator(model, typeMap)
    const derivativesCode = rk4Generator.generate()
    if (derivativesCode) {
      source += derivativesCode
      source += '\n'
    }
    
    // Step function with integrated RK4 (generated by IntegrationOrchestrator)
    const integrationOrchestrator = new IntegrationOrchestrator(model, typeMap, {
      integrationMethod: this.options.integrationAlgorithm,
      includeComments: this.options.includeDebugComments,
      debugMath: this.options.debugMath
    })
    const debugHelpers = integrationOrchestrator.generateHelpers()
    if (debugHelpers) {
      source += debugHelpers
      source += '\n'
    }
